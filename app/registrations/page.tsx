  ...
  const downloadCSV = async () => {
    try {
      ...
      const csvData = sortedData.map(r => {
        ...
        const names24 = [r.fullName, ...mapped24Candidates, ...regMapped24].map(normalizeName)
      ...
      link.setAttribute('download', `jcl-registrations-with-stats-${new Date().toISOString().split('T')[0]}.csv`)
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-4 flex items-center gap-3 flex-wrap">
          ...
          <button
            onClick={downloadPDF}
            disabled={loading || data.length === 0}
            className="px-3 py-2 rounded-md bg-blue-600 text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Print/PDF
          </button>
